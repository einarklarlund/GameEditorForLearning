...
function removeEntity(entity) {
  ...
  if(!entity.preventFromRemovalFromEntityManager) {
    var customConfiguration = entity.hasOwnProperty("customConfiguration") ? entity.customConfiguration : undefined;

    // create and add a new undoObject to the undoHandler
    var undoObjectProperties = {"target": entity, "action": "remove",
      "currentPosition": Qt.point(entity.x, entity.y), "customConfiguration": customConfiguration};
    ...
    // DONT unregister the entity from the customItemManager
    // because the custom configuration needs to be loaded in case
    // a redo occurs
    // customItemManager.unregisterCustomItem(entity.entityId);
...
function placeEntityAtPosition(mouseX, mouseY) {
  ...
  if(selectedButton.hasOwnProperty("index")) {
    // register the entity if it hasn't been registered already (if it was
    // created with the pen tool).
    if(!customItemManager.isEntityRegistered(entity)) {
      // load the entity's custom configuration before registering it
      entity.isCustomized = true;
      entity.customIndex = selectedButton.index;
      entity.loadCustomConfiguration();
      ...
      customItemManager.registerCustomItem(entityId);
    }
  }
  else {
    // if the new entity isn't customized, we need to make sure that any
    // registration w the entity's id is removed
    customItemManager.unregisterCustomItem(entityId);
  } 
