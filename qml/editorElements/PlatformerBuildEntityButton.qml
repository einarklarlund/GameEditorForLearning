  ...
  onEntityWasBuilt: {
    ...
    if(builtEntity) {
      ...
      var undoObjectProperties = {"target": builtEntity, "action": "create",
        "currentPosition": Qt.point(builtEntity.x, builtEntity.y), customConfiguration: undefined}
      var undoObject = editorOverlay.undoHandler.createUndoObject(undoObjectProperties)
      editorOverlay.undoHandler.push([undoObject])

      // since the new entity isn't customized, we need to make sure that any
      // registration w the entity's id is removed
      // customItemManager.unregisterCustomItem(builtEntityId);
    }
  }
}
