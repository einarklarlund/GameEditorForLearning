...
PlatformerEntityBaseDraggable {
  ...
  // if true, the star power has been collected and is still active. the player kills enemies on collision
  property bool starPowerActive: false
  ...
  PolygonCollider {
    ...
    fixture.onBeginContact: {
      ...
      if(otherEntity.entityType === "item") {
          otherEntity.collect()
          console.log(otherEntity.variationType);

          // on contact with a coin, collect it and increase score
          if(otherEntity.variationType === "coin") {
            score += 1
          }
          // on contact with a mushroom, collect it and make player big
          else if(otherEntity.variationType === "mushroom") {
            isBig = true
          }
          // on contact with a star, collect it and make player invincible
          else if(otherEntity.variationType === "star") {
            starPowerActive = true;
            startInvincibility(starInvincibilityTime)
          }
      }

      if(otherEntity.entityType === "opponent" && starPowerActive) {
          otherEntity.die();
      }
    }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if(jumpForceLeft > 0) {
        ...
        // apply the impulse
        collider.applyLinearImpulse(Qt.point(0, verticalImpulse), collider.body.getWorldCenter())
  ...
  function startJump(isNormalJump) {
      console.log("Player is starting jump");
    if(isNormalJump) {
  ...
  function startInvincibility(interval) {
    ...
    if(starPowerActive) {
      console.log("activating invincibility with star power up");
  ...
  function endInvincibility() {
    ...
    if(starPowerActive) {
      // disable star power if it is enabled
      starPowerActive = starPowerActive ? false : starPowerActive;
