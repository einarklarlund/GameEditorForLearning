...
Item {
  ...
  function createTargetObject() {
    ...
    if(customConfiguration != undefined && customConfiguration != null) {
        properties.isCustomized = true;
        properties.customConfiguration = customConfiguration;
    }
    ...
    // initialize the buildEntity with its custom configuration
    customItemManager.initBuildEntity(target)

    target.resetSizeAndImage();
  }
  ...
  function removeTargetObject() {
      ...
      if(target) {
        ...
        // a redo occurs
        // customItemManager.unregisterCustomItem(target.entityId);
