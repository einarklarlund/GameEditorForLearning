...
Item {
  ...
  function createUndoObject(properties) {
    ...
    if(properties.target.isCustomized > -1) {
        // undoObject.customIndex = properties.target.customIndex;
        undoObject.customConfiguration = properties.target.customConfiguration;
        console.log("[UndoHandler] giving undoobject a customConfiguration of " + JSON.stringify(undoObject.customConfiguration));
    }
    else {
        console.log("[UndoHandler] giving undoobject no customConfiguration");
    }
